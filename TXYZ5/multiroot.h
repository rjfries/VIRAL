...
void CheckRoot(GRID HydroGrid , double tau)
{
	...
	for(k=kl;k<kr;k++)
	{
		...
		y[0] = -(u1*X) - u2*Y - u3*Z + (-e + W)*pow(1 + pow(u1,2) + pow(u2,2) + pow(u3,2),0.5);
		y[1] = -(p3*u2) - p4*u3 - P*u1*(1 + pow(u1,2) + pow(u2,2) + pow(u3,2)) - u1*(p1 + e*(1 + pow(u1,2) + pow(u2,2) + pow(u3,2)) - PI*(1 + pow(u1,2) + pow(u2,2) + pow(u3,2))) + X*pow(1 + pow(u1,2) + pow(u2,2) + pow(u3,2),0.5);
		y[2] = -(p3*u1) - (e + p2 - PI)*u2 - p5*u3 - (e - PI)*u2*(pow(u1,2) + pow(u2,2) + pow(u3,2)) - P*u2*(1 + pow(u1,2) + pow(u2,2) + pow(u3,2)) + Y*pow(1 + pow(u1,2) + pow(u2,2) + pow(u3,2),0.5);
		y[3] = -(p4*u1) - p5*u2 - P*u3 + u3*(p1 + p2 + PI - W) + Z*pow(1 + pow(u1,2) + pow(u2,2) + pow(u3,2),0.5);
	...
	for(k=kl;k<kr;k++)
	{
		...
		double row0 = pi00*u0 - pi01*u1- pi02*u2 - pi03*u3;
		double row1 = pi01*u0 - pi11*u1- pi12*u2 - pi13*u3;
		double row2 = pi02*u0 - pi12*u1- pi22*u2 - pi23*u3;
		double row3 = pi03*u0 - pi13*u1- pi23*u2 - pi33*u3;
		
		double trace = pi00 - pi11- pi22 - pi33 ;
...
int conservation_f (const gsl_vector * x, void *params, gsl_vector * f)
{
	...
	const double y0 =  -(u1*X) - u2*Y - u3*Z + (-e + W)*pow(1 + pow(u1,2) + pow(u2,2) + pow(u3,2),0.5);
	const double y1 = -(p3*u2) - p4*u3 - P*u1*(1 + pow(u1,2) + pow(u2,2) + pow(u3,2)) - u1*(p1 + e*(1 + pow(u1,2) + pow(u2,2) + pow(u3,2)) - PI*(1 + pow(u1,2) + pow(u2,2) + pow(u3,2))) + X*pow(1 + pow(u1,2) + pow(u2,2) + pow(u3,2),0.5);
	const double y2 = -(p3*u1) - (e + p2 - PI)*u2 - p5*u3 - (e - PI)*u2*(pow(u1,2) + pow(u2,2) + pow(u3,2)) - P*u2*(1 + pow(u1,2) + pow(u2,2) + pow(u3,2)) + Y*pow(1 + pow(u1,2) + pow(u2,2) + pow(u3,2),0.5);
	const double y3 = -(p4*u1) - p5*u2 - P*u3 + u3*(p1 + p2 + PI - W) + Z*pow(1 + pow(u1,2) + pow(u2,2) + pow(u3,2),0.5);
...
int conservation_df  (const gsl_vector * x, void *params, gsl_matrix * J)
{
	...
	const double df00 =  -pow(1 + pow(u1,2) + pow(u2,2) + pow(u3,2),0.5);
	const double df01 =  -X + u1*(-e + W)*pow(1 + pow(u1,2) + pow(u2,2) + pow(u3,2),-0.5);
	const double df02 =  -Y + u2*(-e + W)*pow(1 + pow(u1,2) + pow(u2,2) + pow(u3,2),-0.5);
	const double df03 =  -Z + u3*(-e + W)*pow(1 + pow(u1,2) + pow(u2,2) + pow(u3,2),-0.5);


	const double df10 =  -((1 + a)*u1*(1 + pow(u1,2) + pow(u2,2) + pow(u3,2)));
	const double df11 =  -p1 - (e + P - PI)*(1 + 3*pow(u1,2) + pow(u2,2) + pow(u3,2)) + u1*X*pow(1 + pow(u1,2) + pow(u2,2) + pow(u3,2),-0.5);
	const double df12 =  -p3 - 2*(e + P - PI)*u1*u2 + u2*X*pow(1 + pow(u1,2) + pow(u2,2) + pow(u3,2),-0.5);
	const double df13 =  -p4 - 2*(e + P - PI)*u1*u3 + u3*X*pow(1 + pow(u1,2) + pow(u2,2) + pow(u3,2),-0.5);
	
	
	const double df20 = -((1 + a)*u2*(1 + pow(u1,2) + pow(u2,2) + pow(u3,2)));
	const double df21 = -p3 - 2*(e + P - PI)*u1*u2 + u1*Y*pow(1 + pow(u1,2) + pow(u2,2) + pow(u3,2),-0.5);
	const double df22 = -p2 - (e + P - PI)*(1 + pow(u1,2) + 3*pow(u2,2) + pow(u3,2)) + u2*Y*pow(1 + pow(u1,2) + pow(u2,2) + pow(u3,2),-0.5);
	const double df23 = -p5 - 2*(e + P - PI)*u2*u3 + u3*Y*pow(1 + pow(u1,2) + pow(u2,2) + pow(u3,2),-0.5);
  
  	const double df30 = (-a*u3); 
	const double df31 = -p4 + u1*Z*pow(1 + pow(u1,2) + pow(u2,2) + pow(u3,2),-0.5);
	const double df32 = -p5 + u2*Z*pow(1 + pow(u1,2) + pow(u2,2) + pow(u3,2),-0.5);
	const double df33 = -P + p1 + p2 + PI - W + u3*Z*pow(1 + pow(u1,2) + pow(u2,2) + pow(u3,2),-0.5);
...
void MultiRootSearchForEnVelUsingDerivatives(GRID HydroGrid , double tau)
{
	...
	for(k=kl;k<kr;k++)
	{
		...
		{		
			...
			HydroGrid[i][j][k].u[0] =  sqrt(1+u1*u1+u2*u2+u3*u3);   //find the new u3 from root finding algorithm
		}
