...
double s95p_p(double e) //e in 1/fm^4
{
	...
	if(e<E1)
		ret = GEVFM*(0.3299*(exp(0.4346*e) -1 ));
	else
	if(e>E1 && e<E2)
		ret = GEVFM*(1.024E-7*exp(6.041*e) + 0.007273 + 0.14578*e);
	else
	if(e>E2 && e<E3)
		ret = GEVFM*(0.30195*exp(0.31308*e) - 0.256232);
	else
	if(e>E3 && e<E4)
		ret = GEVFM*(0.332*e - 0.3223*exp(0.4585) - 0.003906*e*exp(-0.05697*e) + 0.1167*exp(-1.233) + 0.1436*e*exp(-0.9131*e) );
	else
		ret = GEVFM*(0.3327*e  - 0.3223*exp(0.4585) - 0.003906*e*exp(-0.05697*e) );
	
	return ret; //p in 1/fm^4
}
...
double s95p_a(double e) //speed of sound squared --- dp/de
{
	...
	if(e>E3 && e<E4)
		ret = 1*( 0.332  - 0.3223*0.4585*exp(0.4585) - 0.003906*( exp(-0.05697*e) - 0.05697* e*exp(-0.05697*e)) +  0.1436*( exp(-0.9131*e) - 0.9131*e*exp(-0.9131*e) )  );
	else
		ret = 1*( 0.3327 - 0.3223*0.4585*exp(0.4585) - 0.003906*( exp(-0.05697*e) - 0.05697* e*exp(-0.05697*e))  );
