...
void CalcNumVel(GRID HydroGrid, double tau)
{
	...
	for(k=0;k<ZCM;k++)
	{
		...
		if(l==0)
		{
			HydroGrid[i][j][k].NVx[l] = ( p2 + u1*u0*(e+P-PI) )/HydroGrid[i][j][k].T00;
			HydroGrid[i][j][k].NVy[l] = ( p3 + u2*u0*(e+P-PI) )/HydroGrid[i][j][k].T00;
			HydroGrid[i][j][k].NVz[l] = ( p4 + u3*u0*(e+P-PI) )/HydroGrid[i][j][k].T00;		
		}
		else
		{
